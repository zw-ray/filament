...
ShaderCompilerService::OpenGLProgramToken::~OpenGLProgramToken() {
    ...
    // created program is transferred to `OpenGLProgram`. However, these GL resources may persist if
    // the token was cancelled. This ensures thorough cleanup for the case.
        ...
        case Mode::THREAD_POOL: {
            mCompilerThreadPool.queue(priorityQueue, token,
                    [this, &gl, program = std::move(program), token]() mutable {
                        // If the token is canceled, we exit early. If the cancellation occurs
