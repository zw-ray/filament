...
void WebGPUDriver::readPixels(Handle<HwRenderTarget> sourceRenderTargetHandle, const uint32_t x,
    ...
    wgpu::TexelCopyTextureInfo source{ .texture = srcTexture,
        .mipLevel = 0,
        .origin = { x, flippedY, 0 } };
    ...
    auto userData = std::make_unique<UserData>(UserData{ .pbd = std::move(pixelBufferDescriptor),
    ...
    userData->buffer.MapAsync(
            ...
            [](wgpu::MapAsyncStatus status, const char* message, UserData* userdata) {
                ...
                data->that->scheduleDestroy(std::move(data->pbd));
            },
            userData.release());
}
